 ...
 */
        ...
        static void Main(string[] args)
        {
            ...
            int i;//ciklusváltozó
            for (int k = 1; k < fajlbol.Length; k++)
            {
...
/*5.	Jelenítse meg a képernyőn, hogy hány éves volt Agatha Christie, amikor első könyve megjelent! */
            ...
            Console.WriteLine("5. feladat: Agatha Christie {0} évesen jelentette meg első könyvét.", adatok[minindex].kiadaseve-szulev);
 ...
 * A fájlban minden könyv külön sorban szerepel-jen, adatait tabulátorral válassza el egymástól!*/
            ...
            StreamWriter fki = new StreamWriter("1945.txt");
            for (i = 0; i < konyvekszama; i++)
            {
                if (adatok[i].kiadaseve <=1945)
                {
                    fki.Write("{0}\t", adatok[i].angolcim);
                    fki.Write("{0}\t", adatok[i].magyarcim);                   
                    fki.WriteLine("{0}", adatok[i].kiadaseve);                    
                }
            }
            fki.Close();
 ...
 * melyet Agatha Christie a II. vi-lágháború előtt jelentetett meg (1940)! 
 ...
 * Jelenítse meg a képernyőn a 15 legdrágább könyv minden adatát tabulátorral elválasztva!9. feladat:*/
            ...
            for (i = 0; i < 14; i++)
            {
                Console.WriteLine($"{adatok[i].kategoria,-5} {adatok[i].magyarcim,-40} {adatok[i].angolcim,-40} {adatok[i].kiadaseve,-10} {adatok[i].ar}");
            }
            Console.ReadKey();
        }
    }
